 ...
 */
class WhileAction(condition: Expression[Boolean], val next: ActorRef, val counterName: String) extends Action with TimerBasedIterationHandler with CounterBasedIterationHandler with Bypassable {

 ...
 */
class While(condition: Expression[Boolean], val counterName: String, val next: ActorRef) extends Action with TimerBasedIterationHandler with CounterBasedIterationHandler with Bypassable {
